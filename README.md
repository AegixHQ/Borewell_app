...
| | App | Stack | Status |
|---|---|---|---|
| 🌐 | **`apps/web-app`** | React + Vite + React Router | One unified app for all 4 roles (`ADR-0002`) — designed screens per role (`ADR-0005`), real API wiring, map-based site picking |
| 🧰 | **`apps/shared-ui`** | Plain JS | API client + auth helpers shared by `web-app` |
...
> **This repo is the backend + the web app.** The mobile app
> (Expo/React Native, same design and same contracts) lives in its own repo
> with its own copy of the backend. Treat *this* repo's `services/` as the
> source of truth and re-sync the other rather than editing both.
...
Real renders of the running apps, not mockups.

| | |
|---|---|
| **Design direction** | [`docs/assets/screenshots/`](docs/assets/screenshots/) — the approved "Field Console" direction the two apps share |

Still missing: a short demo video of the customer loop
(location → quote → payment → tracking), and screenshots of `web-app` at
desktop width. Capture those from a real run rather than a mockup.
